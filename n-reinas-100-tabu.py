...
def calcular_ataques(s):
    ...
    diag_inversa = [0] * (2 * n - 1)
    
    # Llenar las tablas en un solo paso O(N)
    for col, fila in enumerate(s):
        filas[fila] += 1
        diag_principal[fila - col + (n - 1)] += 1
        diag_inversa[fila + col] += 1
    
    # Contar ataques basados en las frecuencias
    for i in range(2 * n - 1):
        if i < n:
            if filas[i] > 1:
                ataques += (filas[i] * (filas[i] - 1)) // 2
        
        if diag_principal[i] > 1:
            ataques += (diag_principal[i] * (diag_principal[i] - 1)) // 2
            
        if diag_inversa[i] > 1:
            ataques += (diag_inversa[i] * (diag_inversa[i] - 1)) // 2
